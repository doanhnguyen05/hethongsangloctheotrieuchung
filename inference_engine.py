...
# giảm bớt hiện tượng 1 bệnh cùng nhóm nuốt hết bệnh còn lại
GROUP_COMPETITION_PENALTY = 0.08

# phạt khi thiếu core symptom
CORE_MISSING_PENALTY = 0.20

# thưởng thêm khi có cụm triệu chứng rất đặc hiệu
PAIR_SYNERGY = {
    "covid": [
        ({"S03", "S05"}, 0.16),               # ho khan + mất vị giác
        ({"S02", "S03", "S04"}, 0.12),       # sốt cao + ho khan + khó thở
        ({"S06", "S07"}, 0.05),              # đau mỏi cơ + đau đầu
    ],
    "influenza": [
        ({"S02", "S06", "S07"}, 0.18),       # sốt cao + đau mỏi cơ + đau đầu
        ({"S06", "S07"}, 0.08),
    ],
    "common_cold": [
        ({"S08", "S09"}, 0.16),              # đau họng + sổ mũi
        ({"S03", "S09"}, 0.08),
    ],
    "acute_pharyngitis": [
        ({"S08", "S01"}, 0.14),              # đau họng + sốt nhẹ
        ({"S08", "S03"}, 0.06),
    ],
    "pneumonia": [
        ({"S02", "S04"}, 0.14),              # sốt cao + khó thở
        ({"S02", "S04", "S23"}, 0.18),       # + đờm xanh vàng
        ({"S21", "S23"}, 0.06),
    ],
    "bronchial_asthma": [
        ({"S04", "S20"}, 0.18),              # khó thở + khò khè
        ({"S21", "S22"}, 0.10),              # nặng ngực + ho về đêm
    ],
    "tuberculosis": [
        ({"S16", "S17"}, 0.18),              # ho kéo dài + ho ra máu
        ({"S16", "S18"}, 0.16),              # ho kéo dài + sụt cân
        ({"S16", "S19"}, 0.10),              # ho kéo dài + sốt về chiều
        ({"S16", "S18", "S84"}, 0.14),       # + đổ mồ hôi đêm
    ],
    "lung_cancer": [
        ({"S17", "S18"}, 0.14),              # ho ra máu + sụt cân
        ({"S99", "S100"}, 0.12),             # đau ngực dai dẳng + khàn tiếng
        ({"S17", "S99"}, 0.12),
    ],
    "measles": [
        ({"S02", "S25"}, 0.16),              # sốt cao + ban từ mặt xuống
        ({"S25", "S26"}, 0.14),
    ],
    "hand_foot_mouth": [
        ({"S27", "S28"}, 0.18),              # loét miệng + phỏng nước tay chân
        ({"S28", "S92"}, 0.10),
    ],
    "otitis_media": [
        ({"S90", "S91"}, 0.16),              # đau tai + chảy dịch tai
    ],
    "gastritis": [
        ({"S14", "S12"}, 0.10),
    ],
    "gerd": [
        ({"S38", "S39"}, 0.18),              # ợ nóng + trào ngược
        ({"S38", "S40"}, 0.12),
    ],
    "acute_hepatitis": [
        ({"S29", "S30"}, 0.18),              # vàng da + vàng mắt
        ({"S29", "S31"}, 0.12),
        ({"S30", "S31"}, 0.12),
    ],
    "gallstones": [
    ...
    "stomach_cancer": [
        ({"S102", "S103"}, 0.18),            # nôn máu + phân đen
        ({"S14", "S18"}, 0.10),
    ],
    "colorectal_cancer": [
        ({"S108", "S109"}, 0.20),            # thay đổi đi cầu + phân máu
    ],
    "heart_failure": [
        ({"S41", "S42"}, 0.18),              # khó thở khi nằm + mệt khi vận động
        ({"S35", "S41"}, 0.12),              # phù chân + khó thở khi nằm
    ],
    "heart_attack": [
        ({"S45", "S46"}, 0.22),              # đau ngực trái + lan hàm/tay trái
        ({"S45", "S04"}, 0.10),
    ],
    "stroke": [
        ({"S47", "S48"}, 0.22),              # liệt mặt + yếu nửa người
        ({"S47", "S49"}, 0.18),
        ({"S48", "S49"}, 0.18),
    ],
    "anemia": [
        ({"S44", "S43"}, 0.14),              # xanh xao + hoa mắt đổi tư thế
        ({"S44", "S42"}, 0.10),
    ],
    "hypertension": [
        ({"S70", "S07"}, 0.08),
    ...
    "sciatica": [
        ({"S50", "S51"}, 0.16),              # đau lan chân + tê bì
    ],
    "gout": [
        ({"S52", "S53"}, 0.08),
    ],
    ...
    "diabetes": [
        ({"S57", "S58", "S59"}, 0.20),       # ăn nhiều + uống nhiều + tiểu nhiều
    ],
    "hyperthyroidism": [
        ({"S60", "S61"}, 0.14),
        ({"S61", "S62"}, 0.12),
...
# dấu hiệu đi ngược logic của một số bệnh
...
# triệu chứng rất đặc hiệu: nếu bệnh có mà không có các dấu hiệu này thì nên bị trừ nhiều hơn
HIGH_SPECIFICITY_SYMPTOMS = {
    "S05",   # mất vị giác
    "S17",   # ho ra máu
    "S25",   # ban từ mặt xuống thân
    "S27",   # loét miệng
    "S28",   # phỏng nước tay chân
    "S29", "S30", "S31",  # vàng da/vàng mắt/tiểu sẫm
    "S45", "S46",
    "S47", "S48", "S49",
    "S52",
    "S57", "S58", "S59",
...
def _compute_support_scores(selected_symptoms, disease):
    ...
    # precision phải đo trên toàn bộ input user, không chỉ trên weights của bệnh
    ...
    if matched_codes:
        ...
        if non_core_codes:
            supportive_score = min(
                sum(weights.get(code, 0) for code in non_core_codes) / max(total_weight, 1),
                1.0
            )
...
def _compute_specificity_penalty(disease, selected_symptoms, matched_codes):
    """
    Nếu user có triệu chứng rất đặc hiệu mà bệnh này không lý giải được,
    bệnh phải bị trừ mạnh hơn.
    """
    selected_set = set(selected_symptoms)
    matched_set = set(matched_codes)
    missed_specific = [s for s in selected_set if s in HIGH_SPECIFICITY_SYMPTOMS and s not in matched_set]
    return min(len(missed_specific) * 0.04, 0.20)


def _compute_logic_bonus(disease_id, scores, selected_symptoms):
    bonus = 0.0
    ...
    bonus += _compute_pair_synergy(disease_id, selected_symptoms)
    return bonus


def _compute_logic_penalty(disease_id, disease, scores, selected_symptoms):
    penalty = 0.0
    ...
    penalty += _compute_specificity_penalty(disease, selected_symptoms, scores["matched_codes"])
...
def _normalize_group_probabilities(results):
    ...
    adjusted_results.sort(
        key=lambda x: (x["raw_percent"], x["core_percent"], x["coverage_percent"], x["precision_percent"]),
        reverse=True
    )
...
def infer_disease(user_symptoms, age=None, gender="all", severity="moderate", duration="fewdays", followup_adjustments=None):
    ...
    for disease_id, disease in DISEASES.items():
        ...
        supportive_score = scores["supportive_score"]
        missing_core_ratio = scores["missing_core_ratio"]

        bonus = _compute_logic_bonus(disease_id, scores, selected_symptoms)
        logic_penalty = _compute_logic_penalty(disease_id, disease, scores, selected_symptoms)

        confidence = (
            (coverage_score * 0.30)
            + (precision_score * 0.17)
            + (core_score * 0.36)
            + (supportive_score * 0.17)
            + bonus
            - logic_penalty
        )
        ...
        missing_core = [SYMPTOMS[code] for code in disease.get("core", []) if code not in selected_symptoms and code in SYMPTOMS]
        ...
        results.append({
            ...
            "coverage_percent": _round_percent(coverage_score * 100),
            "precision_percent": _round_percent(precision_score * 100),
            "core_percent": _round_percent(core_score * 100),
            "supportive_percent": _round_percent(supportive_score * 100),
            "missing_core_percent": _round_percent(missing_core_ratio * 100),
            "advice": disease["advice"],
            "age_note": age_note,
            "gender_note": gender_note,
            "flags": list(dict.fromkeys(flags)),
            "doctor_summary": (
                f"Bác sĩ AI ghi nhận {len(matched)} triệu chứng phù hợp với {disease['name']}. "
                f"Hệ thống ưu tiên mạnh các bệnh có triệu chứng cốt lõi và cụm triệu chứng đặc hiệu trùng khớp, "
                f"đồng thời giảm điểm các bệnh không giải thích được các dấu hiệu đặc hiệu của hồ sơ hiện tại. "
                f"Điểm trước chuẩn hóa của bệnh này là {raw_percent}%."
            ),
            "explanation": (
                f"Điểm được tính từ độ phủ triệu chứng ({_round_percent(coverage_score * 100)}%), "
                f"độ tập trung triệu chứng ({_round_percent(precision_score * 100)}%), "
                f"mức khớp triệu chứng cốt lõi ({_round_percent(core_score * 100)}%), "
                f"triệu chứng hỗ trợ ({_round_percent(supportive_score * 100)}%), "
                f"bonus do cụm triệu chứng đặc hiệu và trừ điểm nếu không lý giải được triệu chứng đặc hiệu hoặc thiếu core symptom."
            ),
            "formula": {
                "coverage": _round_percent(coverage_score * 100),
                "precision": _round_percent(precision_score * 100),
                "core": _round_percent(core_score * 100),
                "supportive": _round_percent(supportive_score * 100),
    ...
    results = [item for item in ranked_results if item["raw_percent"] >= MIN_DISPLAY_THRESHOLD]
    ...
    if not results and ranked_results:
        ...
        for item in results:
            ...
            item["doctor_summary"] = (
                f"Bác sĩ AI đã ghi nhận một số dấu hiệu bước đầu liên quan tới {item['name']}, "
                f"nhưng dữ kiện hiện tại còn ít hoặc chưa đủ đặc hiệu để kết luận chắc hơn. "
                f"Hệ thống vẫn giữ bệnh này trong danh sách chẩn đoán phân biệt để tham khảo."
            )
            item["explanation"] = (
                f"Bệnh này vẫn được giữ lại vì có ít nhất một phần triệu chứng trùng khớp với hồ sơ đầu vào. "
                f"Tuy nhiên mức khớp hiện còn thấp, nên tỷ lệ chỉ mang ý nghĩa định hướng ban đầu và cần thêm triệu chứng hoặc câu hỏi làm rõ."
            )
    ...
    for item in results:
        ...
        item["relative_level"] = _round_percent((item["display_percent"] / top_score) * 100) if top_score else 0
        item["differential_percent"] = _round_percent((item["display_percent"] / total_display) * 100) if total_display else 0
